...
The next real test isn't more synthetic scenarios — it's running the same engine against one real (even partial, anonymized) planning cycle to see whether the criticality ranking and root-cause taxonomy survive contact with messier real-world data, rather than continuing to validate against scenarios designed to be found.
